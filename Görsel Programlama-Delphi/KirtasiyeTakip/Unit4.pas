...
begin
ShowMessage('Boþ Alan Geçemezsiniz...');
end
else
begin
cevap:=Application.MessageBox('Onaylýyor Musunuz?','Uyarý',MB_YESNO);
if(cevap=IDYES) then
begin
Table1.Insert;
...
begin
ShowMessage('Stok Giriþi Kaydý Yoktur...');
end
else
begin
ShowMessage(IntToStr(Query1.RecordCount)+' Tane Stok Giriþi Listelendi...');
end;
...
begin
ShowMessage('Stok Çýkýþ Kaydý Yoktur...');
end
else
begin
ShowMessage(IntToStr(Form1.Querycikis.RecordCount)+' Tane Stok Çýkýþý Listelendi...');
end;
end;
end;
...
begin
cevap:=Application.MessageBox('Silmek Ýstiyor Musunuz?','Stok Sil',MB_YESNO);
if(cevap=IDYES) then
begin
Table1.Delete;
Table1.Refresh;
...
begin
ShowMessage('Ürün Seçilmemiþtir...');
end;
end;
