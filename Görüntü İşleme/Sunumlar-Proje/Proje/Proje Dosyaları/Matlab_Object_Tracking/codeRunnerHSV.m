...
% durdurulup, tekrar F5 ile kaldýðý yerden devam edilebilir
if(~exist('video','var'))
    ...
    % Durdurma Butonu
    isRunning = 1;
    u=uicontrol('String','Stop','Callback','isRunning = 0; disp(''Hesaplamalar Durduruldu.'')','ForegroundColor','w','BackgroundColor','r','Fontsize',14,'FontWeight','Demi','Position',[1 1 60 60]);
...
% ("doc framegrabinterval" komutundaki þeklin incelenmesi bu bölümün anlaþýlmasý için faydalý olmaktadýr.)
...
centroid = cell(16,1);
area = zeros(16,2);
% HSV Uzayýnda mavi rengin threshold deðerleri
minTh = 0.55;
maxTh = 0.66;

while(video.FramesAcquired>=0)
    data = getsnapshot(video);
    img = rgb2hsv(data);
    % H ve S katmanlarýnýn ayýklanmasý
    H = img(:,:,1);
    S = img(:,:,2);
    % H ve S katmanlarýna thresholding iþlemi
    H(H<minTh | H>maxTh) = 0;
    H(H>minTh & H<minTh) = 1;
    S(S<0.3) = 0;
    S(S>0.3) = 1;
    H = bwareaopen(H,(imgSize)/1200); % Küçük objelerin resimden ayýklanmasý (Resim alanýn 1200'de birinden küçük objeler atýlmaktadýr)
    
    data(H>0) = 255; % Bulunan mavi bölgelerin resim üzerine boyanmasý
        ...
        for object = 1:length(stats) % Bulunan her obje için
            if(object < 16 && object > 0) % Obje sayýsý sýnýrlamasý
                ...
                if(length(stats) == 1) % Tek obje bulunmasý durumunda
                    % Objenin konumlarýnýn bulunmasý
                    x = boundingBox{object}(1);
                    y = boundingBox{object}(2);
                    % Gürültüden veya objenin engel arkasýna gitmesinden dolayý alandaki deðiþikliklerin ekarte edilmesi
