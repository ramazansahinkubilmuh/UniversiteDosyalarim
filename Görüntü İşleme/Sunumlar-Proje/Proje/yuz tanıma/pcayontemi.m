...
function [taninma, dbadi, recognized_img]=pcayontemi(datapath,test_image) %Veritabanýnýn ve test resminin alýndýðý kýsým
D = dir(datapath); 
imgcount = 0;
for i=1 : size(D,1)
...
X = [];
for i = 1 : imgcount
    str = strcat(datapath,'\',int2str(i),'.jpg');  %Veritabanýndaki resimlerin okunup ilk olarak griye sonra tekrar þekillendirilip gecici olarak x e atýldýðý kýsým
    img = imread(str);
    img = rgb2gray(img);
    [r c] = size(img);
